...
installShinyParallel <- function(appDir=getwd(),
    ...
    if (file.access(shinyServerPath, mode=2) != 0)
        stop(paste0('Current user cant write to ', shinyServerPath,
                    ' path. Maybe run it as root: "sudo R"'))
    
    if (!file.exists(shinyServerPath))
        stop(paste0('Is shiny server installed? Can not find server path: ',
                    shinyServerPath))
    ...
    dir.create(toPath, showWarnings=F);
    invisible(lapply(dir(serverFiles, full.names=!F), function(x) file.copy(x, toPath)))
