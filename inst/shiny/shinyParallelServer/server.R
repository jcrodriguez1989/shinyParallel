  ...
  if ('admin' %in% names(isAdmin)) {
    output$stattable <- renderDataTable({
      ...
      if (is.null(stattable))
        stattable <- data.frame(PID='NULL', Port='NULL',
                                Users='0');
      return(stattable)
    });
  } else {
    proc <- assignProcess(portsStatus, processes,
    ...
    if (length(proc) == 1 && is.na(proc)) {
      showModal(modalDialog('Retrying...', title='Server is full.',
                            footer=NULL));
      ...
      output$htmlSess <- renderUI(
        shiny::tags$iframe(
          ...
          style=paste('top: 0', 'left: 0', 'width: 100%', 'height: 100%',
                      'position: absolute', 'border: none', sep='; ')
          )
      );
